...
\title{COMP3211 Tutorial 5: Satisfiability}
\institute{Department of CSE \\ HKUST \\ {\copyright\ 2024 Fengming Zhu. All rights reserved.}}
\date{Mar. 11\&14, 2024} % you can change it to the latest date
...
\section{Boolean Satisfiability Problems (SAT)}
...
\begin{frame}{Preliminaries}
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.8\linewidth]{pic/sat.png}
    \end{figure}
\end{frame}

\subsection{Solving SAT as Search}

\begin{frame}{Solving SAT as Search}
    \begin{exampleblock}{Reduce to SEARCH formulation:}
        \begin{itemize}[<+-| alert@+>]
            \item States: any partial assignment of the variables in $\mathcal{X}$.
            \item Initial state: empty assignment $\{\}$.
            \item Operators: $let(x,v)$ that assigns variable $x$ value $v$ (1/0). This operator is applicable in state $s$ if and only if the
            variable $x$ does not have a value in $s$.
            \item Operator cost: 1 for each operator.
            \item Goal states: those that make all clauses in $\mathcal{C}$ true.
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
%\subsection{Example 1: $A^*$ for SAT}
%
%\begin{frame}{Example 1: $A^*$ for SAT}
%    Now consider the following heuristic function $h(s)$:
%    \begin{itemize}
%      \item If $s$ satisfies all clauses
%        in $\mathcal{C}$, then $h(s)=0$;
%      \item If $s$ makes one of the clauses
%        in $\mathcal{C}$ false, then $h(s)= \infty$;
%      \item otherwise, $h(s)$ is the smaller of the following two numbers: the number of variables not in $s$, and the number of clauses in $\mathcal{C}$ whose truth values are still unknown.
%    \end{itemize}
%    
%    \pause
%    \alert{For example}, if $\mathcal{C} = \{ x_1\lor x_2, \neg x_1\lor \neg x_2, x_2\}$, then
%    \begin{itemize}
%        \item $h(\{\}) = 2$,
%        \item $h(\{x_1=1,x_2=1\})=\infty$,
%        \item $h(\{x_1=0,x_2=1\})=0$.
%    \end{itemize}
%\end{frame}
%
%
%\begin{frame}{Example 1: $A^*$ for SAT (cont'd)}
%Do $A^*$ search for the following set of clauses
%\begin{eqnarray*} \mathcal{C} = \{
...
\section{Constraint Satisfiability Problems (CSP)}
...
\begin{frame}{Formulation}
    \begin{exampleblock}{Notations:}
        \begin{itemize}
            \item A set of Variables $\mathcal{X}$

            \item Each variable $x_i \in \mathcal{X}$ is associated with a set of domain values $\mathcal{V}_i$

            \item A set of constraints (conditions) on variables, could be unitary, binary or else.

            \item \alert{A solution is an assignment where all variables get assigned and all constraints are satisfied.}
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Constraints Propagation}
    \begin{exampleblock}{How to do search:}
        \begin{itemize}[<+-| alert@+>]
            \item Naive way: backtracking search (depth-first search) 

            \item Make use of constraints -- constraints propagation
            \begin{figure}[htpb]
                \centering
                \includegraphics[width=0.8\linewidth]{pic/constructive.png}
            \end{figure}

            \item Furthermore: arc consistency
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\subsection{Example: Sudoku as CSP}

\begin{frame}{Example: Sudoku as CSP}
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.3\linewidth]{pic/sudoku.png}
    \end{figure}

    \begin{exampleblock}{As a CSP:}
        \begin{itemize}[<+-| alert@+>]
            \item Variables: $X_{ij}$, for ungiven $i, j \in [1..9]$

            \item Domain values: $X_{ij} \in [1..9]$ for all ungiven $i, j \in [1..9]$

            \item Constraints: each row, column and square has DISTICNT $[1..9]$
        \end{itemize} 
    \end{exampleblock}

\end{frame}

\begin{frame}{Example 2: Sudoku as CSP (cont'd)}
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.3\linewidth]{pic/sudoku.png}
        \includegraphics[width=0.42\linewidth]{pic/sudoku_result.png}
    \end{figure}
